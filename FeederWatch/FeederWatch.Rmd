...
```{r}
libs <- c("tidyverse","dplyr", "maps", "Amelia", "ggrepe", "usmap", "RColorBrewer","tidymodels", "vip")
...
I've decided to replace NA value in dataset for 1988 with median for group like I did for the primary dataset. I've started with the data for year 1988 but there was a problem - I couldn't calculate median because in column `SNOW_DEP_ATLEAST` for 1988 were only NA! 
It has occurred to me that in 1988 participants didn't have to estimate snow cover. It was better for 1989 because it was only 280611 records with NA value for snow depth.
...
As before, I've decided to compare data from 2021 with different year - in this case, I compared with 2011 to check differences.

```{r}
feederwatch_2001_2005 <- readr::read_csv('https://clo-pfw-prod.s3-us-west-2.amazonaws.com/data/PFW_2011_2015_public.csv')

feederwatch_2011 <- feederwatch_2011_2015 %>% 
  filter()
```
I've chosen data for 2011.
...
```
I've chosen 3 "different" kind of bird -, `yerwar` (Yellow-rumped Warbler), `cedwax` (Cedar Waxwing
), `lessca` (Lesser Scaup), for purpose to show 3 different distribution. Based on generated plot I've had the following conclusions:
...
I've decided to work with site data, where each row represents one observation and their associated site description. 

The first step in building regression models is to split original data into a training and test set. I've performed then  all feature engineering and model fitting tasks on the training set and I've used the test set as an independent assessment of my model’s prediction accuracy.
