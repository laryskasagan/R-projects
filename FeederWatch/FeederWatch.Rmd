...
I've dropped 5 columns that I decided weren't providing significant information. Then I explored how much missing data I have in the dataset. I've genereted a plot showing where missingness occurs in the dataset. I've found that in that dataset I had only 1% of missing data NA - in 1 column which was snow_dep_atleast. I've decided to replace the NA value with calculated median value for each region.
...
In the below chunk I've prepared data in order to plot the results. I've used the `usmap` package in combination with the `ggplot2` package to generate map of the United States of America (USA). On the generated plot, I've intended to show the depth of the snow cover depending on the state.
...
Our expectation would be that Alaska would have the highest mean minimum value of snow depth across all states in the USA. Alaska is the most northerly state in the USA so it is logical assumption, which is confirmed by above plot. In the southern states value of mean minimum snow depth is close to the zero, and increases with latitude. This subset of dataset comes from year 2021. I've decided to compare data from other years to check the difference in snowfall between these years\*.
...
I've decided to replace the NA value in the dataset for 1988 with median for group like I did for the primary dataset. I've started with the data for year 1988 but there was a problem - I couldn't calculate median because the column `SNOW_DEP_ATLEAST` for 1988 contained only NA values. It has occurred to me that in 1988 participants didn't have to estimate snow cover. 1989 had better results with "only" 280611 records with the NA value for snow depth.
...
What is surprising, according to the generated plot in 1989 there were much more states with mean of snow depth equal 0 than in 2021. On the other hand, the highest value belongs to Alaska in both cases but in 1989 this value was higher than in 2021.
...
# Data preparation - site_data

For starters, I've decided to generate missing plot. In this dataset I've found 26% of missingness! Most of them are in columns describing numfeeders but there are missingness everywhere except loc_id. I've divided this dataset into smaller datasets with specific information. In the `animals_activity_data` dataset there is only 8% of missingness. I've changed NA to 0 in animals activity set.
...
I've created a dataset called `animals_activity_data` which contains information about animals and nearby feeders - 1 means presence and 0 means absence of specific animal. Based of obtained result, it appears that presence of squirrels is more common than their absence. Similarly, presence of cats, dogs and human is more common than their absence.
...
In the below chunk I've prepared data to visualize how total of bird's population is changing through time in North America.
...
I've created a line plot for only 5 species of birds because dataset has data for 361 different species. It's a lot and any try to generate line plot for every species would make the line plot illegible. I've also decided that population calculation based on weekly observation would give the best result to show on line plot. I had to figure out breaks on x axis by trial and error and in my opinion 2 week breaks are the most readable.
...
In the above excerpt I've created a simple map of the world, where based on latitude and longitude, observations are marked. Below I've focused on the map of North America, where it is clear that more sightings have been made in the eastern part of the continent.
...
Then, after checking the ratio of Canadian observations to all records (it was approximately 18%) I've decided to focus only on American observations.
...
In the chunk below I've prepared a scatter plot which aims to show the distribution and variation of bird observations over time for different bird species, allowing for comparisons and insights into the patterns and trends of bird sightings.
...
I've chosen 3 "different" kinds of bird - `yerwar` (Yellow-rumped Warbler), `cedwax` (Cedar Waxwing ), `lessca` (Lesser Scaup), for purpose to show 3 different distribution. Based on generated plot I've had the following conclusions:

1.  The counts for Yellow-rumped Warbler remain consistent throughout the observed periods. The outlier for this species, which is significantly different from other observations, is 20 but still relatively low compared to other species. Yellow-rumped Warbler are frequently observed - this is one of the most common species in the dataset `feederwatch`.
...
I've worked with the `feederwatch` and `site_data` datasets in these modelings. A row in those datasets represent one observation. For `feederwatch` each observation has included e.g., species. For `site_data` each observation has included e.g., different habitat types.

## Linear regression

### Pre-processing and preparation

Before modeling I had to preprocess and prepare the data. That involved:

1.  Handling missing values

```{r}
site_data_df <- site_data %>% 
select(hab_dcid_woods,hab_evgr_woods,hab_mixed_woods,hab_orchard,hab_park,hab_water_fresh,hab_water_salt,hab_residential,hab_industrial,hab_agricultural,hab_desert_scrub,hab_young_woods,hab_swamp,hab_marsh,nearby_feeders,squirrels,cats,dogs,humans,housing_density) %>% 
   mutate(hab_dcid_woods = replace_na(hab_dcid_woods, 0),
          ...
          dogs = replace_na(dogs, 0),
          humans = replace_na(humans, 0),
          housing_density = replace_na(housing_density, 0)) %>% 
  as.data.frame(.)
```
...
site_data_split <- initial_split(site_data_df, prop = 0.75, strata = housing_density)
...
I've used "housing_density" as response variable to predict density of housing.
...
The code below trains linear regression model on the `feederwatch_linear_regression_training` data. As I've written before, I specified that speciesdaejun is the response variable and the rest of variables are predictor variables.

```{r}
lm_fit <- lm_model %>% 
  fit(housing_density ~ ., data = site_data_training)
...
1.  The coefficient estimates indicate the relationship between each predictor variable and the response variable.

2.  Based on obtained p-values several of the predictor variables are significant. Variables with p-values less than 0.05 (indicated by '\*') are considered statistically significant. For example, dogs have a p-value of 0.0162, suggesting a significant relationship with the housing density.

3.  The model's residual standard error is 0.7903, which represents the average difference between the observed values and the predicted values.

4.  The model's multiple R-squared value is 0.4733, indicating that approximately 47.33% of the variance in housing density can be explained by the predictor variables in the model. The adjusted R-squared value is similar, suggesting that the model's goodness-of-fit remains consistent even when considering the number of predictors.

5.  The F-statistic has a large value of 9021, indicating that the overall model is statistically significant, and the probability of obtaining such a large F-statistic by chance is extremely low.

Overall, the model suggests that several predictor variables, including habitat types, nearby feeders, and the presence of certain animals (squirrels, cats, dogs, and humans) have significant effects on housing density.
...
I've concluded from the results above, that hab_residential, hab_park and hab_agricultural are the 3 most important predictors of site_data for housing density; at the opposite side the 2 least important predictors of site_data for squirrels are hab_swamp and hab_young_woods.

The code below uses the `predict` function to generate a data frame with a single column, `.pred`, which contains the predicted presence of squirrels in the `site_datatest` dataset.

```{r}
lm_fit %>% 
  predict(new_data = site_data_test)

site_data_test_results <- lm_fit %>% 
  predict(new_data = site_data_test) %>% 
  bind_cols(site_data_test)

site_data_test_results
```

To obtain the RMSE and R2 values in the test set results, I've used the `rmse()` and `rsq()` functions.

```{r}
#RMSE
site_data_test_results %>% 
  rmse(
    truth = housing_density,
    estimate = .pred
    )
...
Based on these results, I've concluded the following:

1.  The relatively low RMSE value suggests that the model's predictions have a moderate level of accuracy in estimating the actual housing density. However, the interpretation of the RMSE value depends on the scale and context of the variable being predicted.
2.  The R-squared value of 0.4707583 indicates that the model explains approximately 47.08% of the variability observed in the housing density. This means that other factors not included in the model may also influence the housing density.
...
```{r}
R2_plot <- site_data_test_results %>% 
  ggplot(aes(x = .pred, y = housing_density)) +
    geom_point(color = 'darkred', size = 0.1) +
    geom_abline(intercept = 0, slope = 1, color = 'orange') +
    labs(
      title = 'Linear Regression Results - Site Data Test Set',
      x = 'Predicted housing_density',
      y = 'Actual housing_density'
      )

R2_plot 
```

## Creating a Machine Learning Workflow based on linear regression model

In the previous section, I've trained a linear regression model. In this section, I've gone over how to combine all of the modeling steps into a single workflow.
...
Next, I've specified a feature engineering recipe. This recipe will be automatically applied in later steps.

```{r}
site_data_recipe <- recipe(
  housing_density ~ .,
  data = site_data_training) %>% 
...
In the below chunk, I've specified linear regression model with parsnip.
...
I've used the `workflow` package which was designed to combine models and recipes into a single object. To obtain the performance metrics and predictions on the test set, I've used the `collect_metrics()` and `collect_predictions()` functions on site_data_fit object. I've saved the test set predictions by using the `collect_predictions()` function.

```{r}
site_data_workflow <- workflow() %>% 
  add_model(lm_model) %>% 
  add_recipe(site_data_recipe)

site_data_workflow

site_data_fit <- site_data_workflow %>% 
  last_fit(split = site_data_split)

site_data_fit %>% 
  collect_metrics()

test_results <- site_data_fit %>% 
  collect_predictions()

test_results

model_fitted <- site_data_workflow %>% 
  fit(data = site_data_training)
...
When the majority of the variables in the `site_data` dataset are binary and only one variable has a continuous value ranging, linear regression may not be the most suitable choice. Linear regression assumes a linear relationship between the predictor variables and the response variable. While it can handle binary variables, it may not capture the nonlinear relationships that could exist in data. It might be more appropriate to consider logistic regression rather than linear regression. Logistic regression is specifically designed for binary response variables.

## Decision Trees

### Pre-processing and preparation

Before modeling, I've needed to preprocess and prepare the data. That involved:

1.  Handling missing values.

2.  Encoding categorical variables - feederwatch dataset includes categorical features (e.g.,species_code) which I've converted into numerical representations, such as one-hot encoding using the `caret` package.
...
In such case, I've decided to filter feederwatch dataset and to choose the most numerous species while grouping the remaining species into separate "other" category. This approach helps to reduce dimensionality and prevents overfitting caused by rare categories.
...
3.  Splitting the data - I've divided the dataset into a training set and a testing set. I've used the training set to train the Random Forest model, while the testing set is used to evaluate its performance.
...
Next, I've specified a decision tree classifier with the following hyperparameters:
...
After preparing the model and recipe, I've combined them into a workflow, which allows to easily manage the model-building process.
...
It's important to note that the other combinations of hyperparameters did not perform as well in terms of ROC AUC score. The combination with cost_complexity equal to 1e-10, tree_depth equal 15, and min_n equal 40 had a lower mean ROC AUC score - 0.5274334, while the combinations with tree_depth equal 1 or cost_complexity equal 0.1 resulted in a mean ROC AUC score of 0.5, indicating no predictive power.
...
I've been right and the first model was the best.
...
I've concluded from the results above, that how_many with about 75% importance is the most important predictor of feederwatch. The plot suggests also that snow_dep_atleast is the second strongest predictor but its importance is about 20%. The least accurate predictor is speciesOther with less than 5%.
...
Next I've fitted final model workflow to the training data to evaluate performance of the test data. I've reviewed performance metrics of the test data.
...
1.  The model was able to accurately predict the target variable (valid or not) for the observations in the evaluated dataset.
...
In this section, I've fitted a random forest model to the `feederwatch_df` data. Random forest takes decision trees and constructs more powerful models in terms of prediction accuracy. In this modelling, I've followed similar steps as with the modelling of decision trees.

### Pre-processing and preparation

I've already split data into training, test, and cross validation sets as well as trained feature engineering recipe, feederwatch_recipe. I've been reusing them in random forest workflow.

### Model specification

Next, I've specified a random forest classifier with the following hyperparameters:
...
In the chunk above, I've set the range from 4 to 11. This is because feederwatch_df has 15 columns and I wanted to test `mtry()` values somewhere in the middle between 1 and 15, trying to avoid values close to both ends.
