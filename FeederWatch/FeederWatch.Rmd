...
At chunk below I've prepered data to 
...
missing_states <- setdiff(states,region_dataset_USA_1988$state)
...
snow_depth_plot_to_compare_1988 <- genereting_of_snow_depth_plot_USA(region_dataset_USA_1988,1988)
snow_depth_plot_to_compare_2021 <- genereting_of_snow_depth_plot_USA(region_dataset_USA,2021)

snow_depth_plot_to_compare_1988
snow_depth_plot_to_compare_2021
```
What is surprising, according to the generated plot in 1988 there were much more states with mean of snow depth equal 0 than in 2021. On the other hand, the highest value belongs to Alaska in both case but in 1988 this value was higher than in 2021.

The interpretation of obtained result:

1. Although FeederWatch creator's attempt to minimize errors, a small percentage of FeederWatch reports are incorrect and analysts must be aware that misidentifications, data entry errors, and other sources of error can evade their data validation system. It's logical assumption that in their early years of operating errors were more common. Over time, every organizations tend to evolve and improve their systems and processes based on lessons learned from past errors or inefficiencies. 

2. Minimum snow depth was estimated by participants what can generated discrepancy with obtained results because participants may have different interpretations of what constitutes the minimum depth, leading to inconsistencies in their estimates. They might also used different methods or tools to measure snow depth. And last but not least, for the first years error might be caused by lack of standardization. Without clear guidelines or instructions on how to measure snow depth, participants might have different understandings of what measurement should be considered as the minimum. The absence of standardized procedures can result in inaccurate estimates.

```{r}
snow_dept_diffrences_between_years_1988_2021 <- region_dataset_USA_1988 %>% 
  mutate(diff = ifelse(state %in% region_dataset_USA$state, (mean_snow_dep - region_dataset_USA$mean_snow_dep),NA)) %>% 
  select(state,diff)

snow_dept_diffrences_between_years_1988_2021 %>% filter(diff > 0)
snow_dept_diffrences_between_years_1988_2021 %>% filter(diff <= 0)
```

I've decided to compare result in tibble to see exact difference between these years. For 7 states difference was greater than 0 which means that in 1988 value of mean snow depth was higher than in 2021. For the rest - the value of mean snow depth was higher in 2021 than in 1988. 
