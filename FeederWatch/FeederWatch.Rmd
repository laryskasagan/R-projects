...
2. Based on obtained p-values several of the predictor variables are significant. Variables with p-values less than 0.05 (indicated by '*') are considered statistically significant. For example, dogs has a p-value of 0.0162, suggesting a significant relationship with the housing density.

4. The model's residual standard error is 0.7903, which represents the average difference between the observed values and the predicted values.

5. The model's multiple R-squared value is 0.4733, indicating that approximately 47.33% of the variance in housing density can be explained by the predictor variables in the model. The adjusted R-squared value is similar, suggesting that the model's goodness-of-fit remains consistent even when considering the number of predictors.

6. The F-statistic has a large value of 9021, indicating that the overall model is statistically significant, and the probability of obtaining such a large F-statistic by chance is extremely low.
...
```
I've conclude from the results above, that hab_residential, hab_park and hab_agricultural are 3 the most important predictors of site_data for housing density. 2 the least important predictors of site_data for squirrels are hab_swamp and hab_young_woods.

The code below uses the `predict` function to generate a data frame with a single column, `.pred`, which contains the predicted presence squirrels on the `site_datatest` data.
...
```
To obtain the RMSE and R2 values on test set results, I've used the `rmse()` and `rsq()` functions.
...
In the previous section, I've trained a linear regression model to the site data step-by-step. In this section, I've gone over how to combine all of the modeling steps into a single workflow.
...
Next, I've specified feature engineering recipe. This recipe will be automatically applied in a later steps.
...
At chunk below, I've specified linear regression model with parsnip.
...
I've used `workflow` package which was designed to combine models and recipes into a single object. To obtain the performance metrics and predictions on the test set, I've used the `collect_metrics()` and `collect_predictions()` functions on site_data_fit object. I've saved the test set predictions by using the `collect_predictions()` function. 
